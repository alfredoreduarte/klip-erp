...
class ProductVariantTest < ActiveSupport::TestCase
  ...
  test "should generate lot number automatically" do
    ...
    assert_match /VARIANT-TEST-001-\d{8}-[A-F0-9]{8}/, lot.lot_number
  end

  test "should have display name" do
    @variant.name = "Red Variant"
    assert_equal "Test Product - Red Variant", @variant.display_name

    @variant.name = nil
    assert_equal "Test Product - VARIANT-TEST-001", @variant.display_name
  end
