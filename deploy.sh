...
( docker compose -f $COMPOSE_FILE -p $NEXT_STACK build --build-arg IMAGE_TAG=$TAG )

# Bring up next stack detached
( docker compose -f $COMPOSE_FILE -p $NEXT_STACK up -d )

# Wait for health (basic sleep; enhance with curl check)
sleep 15

# Label new containers as active for Traefik switch
for cid in $(docker compose -f $COMPOSE_FILE -p $NEXT_STACK ps -q); do
  docker container update --label-add klip.active=1 $cid
done
# Remove active label from old stack
for cid in $(docker compose -f $COMPOSE_FILE -p $ACTIVE_STACK ps -q | xargs); do
...
sleep 30
docker compose -f $COMPOSE_FILE -p $ACTIVE_STACK down